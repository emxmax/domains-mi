...
<div id="wpuef-extra-fields-custom-form-<?php echo $random;?>" class="wpuef-extra-fields-custom-form">
	...
	foreach($extra_fields->fields as $extra_field):
		if(!in_array($extra_field->cid, $fields_ids) || !$wpuef_user_model->field_can_be_displayed_for_current_user($extra_field))
			continue;
	...
	?>
			...
			<?php 
				...
				<select class="wpuef_input_select" name="wpuef_options[<?php echo $extra_field->cid; ?>]" <?php if($read_only) echo ' disabled="disabled"'; if($required) echo 'required="required"';?>>
			...
			<?php elseif($extra_field->field_type == "file"): ?>
				<span id="wpuef-file-box-<?php echo $extra_field->cid."-".$random; ?>"> <!--  //file upload edit --><?php
					...
					<?php endif; 
						...
						<span class="wpuef_file_uploader_container">
							<input class="wpuef_input_file input-text" type="file" value=""  
								id="wpuef_upload_field_<?php echo $extra_field->cid."_".$random; ?>"
							   data-id="<?php echo $extra_field->cid."-".$random; ?>"  
							   data-extra-field-custom-form-id="<?php echo $random ?>"
								data-upload-button-id="#wpuef_file_upload_button_<?php echo $extra_field->cid."_".$random; ?>"
							...
							<input type="hidden" id="wpuef-filenameprefix-<?php echo $extra_field->cid."-".$random; ?>" name="wpuef_files[<?php echo $extra_field->cid; ?>][file_name_tmp_prefix]" value=""></input>
							<!-- Upload button -->
							<button class="button wpuef_file_upload_button"  
								   id="wpuef_file_upload_button_<?php echo $extra_field->cid."_".$random; ?>" 
								   data-id="<?php echo $extra_field->cid."-".$random; ?>"     
								   data-extra-field-custom-form-id="<?php echo $random ?>"
								   data-upload-field-id="#wpuef_upload_field_<?php echo $extra_field->cid."_".$random; ?>"
								   data-file-id="<?php echo $extra_field->cid; ?>" ><?php _e('Upload', 'wp-user-extra-fields') ?></button>
							<button class="button wpuef_file_tmp_delete_button"  
									id="wpuef_file_tmp_delete_button_<?php echo $extra_field->cid."-".$random; ?>"
									data-id="<?php echo $extra_field->cid."-".$random; ?>"  
									data-file-to-delete=""><?php _e('Delete', 'wp-user-extra-fields') ?> </button>
							<!-- Upload progress managment -->
							<span id="wpuef_upload_progress_status_container_<?php echo $extra_field->cid."_".$random; ?>" class="wpuef_upload_progress_status_container">
								<span class="wpuef_upload_progressbar" id="wpuef_upload_progressbar_<?php echo $extra_field->cid."_".$random; ?>"></span >
								<span class="wpuef_upload_progressbar_percent" id="wpuef_upload_progressbar_percent_<?php echo $extra_field->cid."_".$random; ?>">0%</span>
							</span>
						</span>
					<?php endif ?>
				</span>
			...
			<?php elseif($extra_field->field_type == "checkboxes"): ?>
				...
				<input type="hidden" value="<?php if(isset($field_value[$index])) echo 'true'; else 'null';?>" id="<?php echo $extra_field->cid."-".$index; ?>" name="wpuef_options[<?php echo $extra_field->cid; ?>][<?php echo $index ?>]" />
					<input type="checkbox" <?php if(isset($field_value[$index])) echo 'checked';?>  value="<?php echo $index ?>" data-id="<?php echo $extra_field->cid."-".$index;?>" <?php if($required) echo 'required="required" class="wpuef_checkbox wpuef_checkbox_perform_check wpuef_checkobox_group_'.$extra_field->cid.'" '; else echo 'class = "wpuef_checkbox"'; ?> <?php if($read_only) echo 'readonly="readonly" ';?> <?php if($read_only) echo 'readonly="readonly" ';?>><span class="wpuef_checkbox_label"><?php echo $extra_option->label; ?></span></input><br/>
				<?php endforeach; ?>
...
<?php if(!$this->extra_fields_custom_from_rendered_at_least_one_time):?>
	...
	var wpuef_num_field_min_max_error = "<?php _e("Please insert a valid number value.", 'wp-user-extra-fields'); ?>";  
	var wpuef_num_field_min_text = "<?php _e("Min:", 'wp-user-extra-fields'); ?>"; 
	var wpuef_num_field_max_text = "<?php _e("Max:", 'wp-user-extra-fields'); ?>"; 

...
<?php foreach($extra_fields->fields as $extra_field):
	...
	<?php if($extra_field->field_type == 'title_no_input'): 
			$tag = !isset($extra_field->title_tag) ? 'legend' : $extra_field->title_tag; 
	...
	<?php elseif($extra_field->field_type == "checkboxes"): ?>
		...
		<input type="checkbox" data-id="<?php echo $extra_field->cid."-".$index;?>" class="wpuef_checkbox wpuef_field <?php if($required) echo "wpuef_checkbox_perform_check wpuef_checkobox_group_".$extra_field->cid ?>"  value="<?php echo $index ?>" <?php if($extra_option->checked) echo 'checked';?>  <?php if($required) echo 'required="required" ';?> ><span class="wpuef_checkbox_label"><?php echo $extra_option->label; ?></span></input><br/>
		<?php endforeach; ?>

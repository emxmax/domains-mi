...
<?php foreach($extra_fields->fields as $extra_field): 
	...
	<?php elseif($extra_field->field_type == "checkboxes"): ?>
		<?php foreach($extra_field->field_options->options as $index => $extra_option): ?>
			...
			<input type="checkbox" <?php if(isset($field_value[$index])) echo 'checked';?>  value="<?php echo $index ?>"  data-id="<?php echo $extra_field->cid."-".$index; ?>" <?php if($required) echo 'required="required"  class="wpuef_checkbox wpuef_checkbox_perform_check wpuef_checkobox_group_'.$extra_field->cid.'" '; else echo 'class = "wpuef_checkbox"';?> <?php if($read_only) echo 'readonly="readonly"   disabled="disabled"';?>><span class="wpuef_checkbox_label"><?php echo $extra_option->label; ?></span></input><br/>
		<?php endforeach; ?>
	...
	<?php elseif($extra_field->field_type != "title_no_input"): ?>
		<input class="input-text wpuef_input_text <?php echo $password_class; ?>" type="<?php if($is_password_override) echo 'password'; else echo $extra_field->field_type; ?>" placeholder="<?php echo $placeholder; ?>" value="<?php if(!$is_password_override) echo $field_value; ?>" name="<?php if($is_password_override) echo $password_field_name; else echo 'wpuef_options['.$extra_field->cid.']'; ?>" <?php if($required && !$is_password_override) echo 'required="required"';?> <?php if($read_only) echo 'disabled="disabled"';?>/>
	<?php endif; 
	//End types
	?>

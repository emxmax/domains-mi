...
function woo_change_order_received_text( $str, $order ) {
    ...
    if(!$is_finished_payment && isset($cip) && !empty($cip)) {
        $new_str = $str . ' Para finalizar esta compra debes acercarte a pagar con el siguiente código de pago: <b>' . $cip . '</b>. Tiene 24h para realizar el pago y enviar el comprobante al correo hola@encremadas.com. ¡Hazlo antes que se acabe el tiempo de expiración! Te enviamos a tu correo la información detallada para que realices el pago.';
     return $new_str;
    ...
    function init_wc_culqi_payment_gateway() {
        ...
        class WC_culqi extends WC_Payment_Gateway
        {
            public function __construct() {
                ...
                $this->description = 'Paga con tu tarjeta de crédito/débito  o PagoEfectivo (nuevo) a través de Culqi. ¡Elige el medio de pago que más te convenga!<p>';
           ...
           function actualizar_orden () {
            ...
            function receipt_page($order_id) {
                ...
                <div id="info_payment">
                    ...
                    <button id="pagar-now" class="shop-button">Pagar</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button id="btn-back" class="shop-button">Cancelar</button>
                ...
                <script>
                                ...
                                success: function(data) {
                                    ...
                                    if (result.object === "error") {
										...
										if (result.object === "charge") {
	                                        $('#notify').empty();
	                                        $("#info_payment").remove();
																					$('div.woocommerce').append("<div class='enc_compra_exito' style='text-align: center;'><h1>Pago realizado exitosamente</h1>" +
	                                        "<p>¡Ya estás casi lista para Encremarte!<br />Tu mail de confirmación con la fecha de entrega de tu producto llegará pronto.</p>" +
	                                        "<br><button id='home' class='shop-button'>Seguir comprando</button></div>");
                    ...
                    $(document).ready(function() {
                        ...
                        $("div.woocommerce").on('click','#home', function(){
                            var url = '<?php echo home_url(); ?>/categorias/';
                            window.location.replace(url);
                        });

        ...
        static function init()
        {
            self::$allFields=array(
                'enable_megamenu'=>array(
                    'label'=>esc_html__('Enable Mega menu','skincare'),
                    'type'=>'checkbox',
                    'depth'=>0,
                ),
                'icon_menu2'=>array(
                    'label'=>esc_html__('Custom width Mega menu. Example "500px".','skincare'),
                    'type'=>'text',
                ),
                'image'=>array(
                    'label'=>esc_html__('Image Preview','skincare'),
                    'type'=>'image',
                ),
                'content2'=>array(
                    'label'=>esc_html__('Content From Mega Item','skincare'),
                    'type'=>'select',
                    'depth'=>0,
                ...
                <div class="wrap-metabox">
                    <div class="live-previews">
                        <?php if(!empty($item_value)):?>
                            <img src="<?php echo esc_url(wp_get_attachment_url($item_value))?>"/>
                        <?php endif;?>
                    </div>
                    <a class="button button-primary sv-button-remove"> <?php esc_html_e("Remove","skincare")?></a>
                    <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload","skincare")?></a>
    ...
    class S7upf_WalkerNavMenu extends Walker_Nav_Menu
    {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)' ,'skincare'), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)','skincare'), $item->title );
            }
            ...
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo s7upf_add_html_attr($submenu_text); ?>><?php esc_html_e( 'sub item' ,'skincare'); ?></span></span>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','skincare'); ?>">&#8593;</abbr></a>
                                |
                                <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                                ...
                                ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','skincare'); ?>">&#8595;</abbr></a>
                            </span>
                            <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','skincare'); ?>" href="<?php
                            ...
                            ?>"><?php esc_html_e( 'Edit Menu Item' ,'skincare'); ?></a>
                        </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                <?php if( 'custom' == $item->type ) : ?>
                    <p class="field-url description description-wide">
                        <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'URL' ,'skincare'); ?><br />
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Navigation Label' ,'skincare'); ?><br />
                        <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                    </label>
                </p>
                <p class="description description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Title Attribute' ,'skincare'); ?><br />
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Open link in a new window/tab','skincare' ); ?>
                    </label>
                </p>
                <?php
                ...
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'CSS Classes (optional)' ,'skincare'); ?><br />
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Link Relationship (XFN)' ,'skincare'); ?><br />
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Description' ,'skincare'); ?><br />
                        <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                        <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.' ,'skincare'); ?></span>
                    </label>
                </p>

                <p class="field-move hide-if-no-js description description-wide">
                    <label>
                        <span><?php esc_html_e( 'Move' ,'skincare'); ?></span>
                        <a href="javascript:void(0)" class="menus-move-up"><?php esc_html_e( 'Up one' ,'skincare'); ?></a>
                        <a href="javascript:void(0)" class="menus-move-down"><?php esc_html_e( 'Down one' ,'skincare' ); ?></a>
                        <a href="javascript:void(0)" class="menus-move-left"></a>
                        <a href="javascript:void(0)" class="menus-move-right"></a>
                        <a href="javascript:void(0)" class="menus-move-top"><?php esc_html_e( 'To the top' ,'skincare'); ?></a>
                    </label>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s','skincare'), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e( 'Remove' ,'skincare'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel','skincare'); ?></a>
                </div>

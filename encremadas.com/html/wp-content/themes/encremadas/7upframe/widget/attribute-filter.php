...
if(!class_exists('S7upf_Attribute_Filter') && class_exists("woocommerce")){
    class S7upf_Attribute_Filter extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('7uptheme Attribute Filter','skincare'),
                array( 'description' => esc_html__( 'Filter product shop page', 'skincare' ), ));
									...
									if(is_array($terms)){
										foreach ($terms as $term){                                
											if(is_object($term)){
												...
												if(!empty($image)){
													echo sprintf(
														'<a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-image swatch-%s %s" data-value="%s" href="%s"><img src="%s" alt="%s"><span class="hide">%s</span></a>',
														esc_attr($attribute),
														esc_attr( $term->slug ),
														esc_attr( $term->slug ),
														$selected,
										...
										foreach ($terms as $term){
											if(is_object($term)){
												...
												if(!empty($color)){
													...
													echo sprintf(
														'<li><a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-color '.$class_white_color.' swatch-%s %s" href="%s"><span class="attr-before"  '.s7upf_add_html_attr('background-color:'.$color).'></span>%s</a><span class="count">%s</span></li>',
														esc_attr($attribute),
														esc_attr( $term->slug ),
														esc_attr( $term->slug ),
														$selected,
									...
									if(is_array($terms)){
										foreach ($terms as $term){
											if(is_object($term)){
												...
												if(!empty($label)){
													echo sprintf(
														'<li><a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-label swatch-%s %s" data-value="%s" href="%s"><span class="attr-before"></span>%s</a><span class="count">%s</span></li>',
														esc_attr($attribute),
														esc_attr( $term->slug ),
														esc_attr( $term->slug ),
														$selected,
									...
									foreach ($terms as $term) {
										if(is_object($term)){
											...
											echo    '<li class="'.esc_attr($term->slug).'-inline">
														...
														<span class="attr-before"></span>'.$term->name.'
														</a>
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Attribute:' ,'skincare'); ?></label></br>
                <select id="<?php echo esc_attr($this->get_field_id( 'attribute' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'attribute' )); ?>">
                    ...
                    else echo esc_html__("No any attribute.","skincare");
                    ?>
                </select>                
            </p>
        <?php

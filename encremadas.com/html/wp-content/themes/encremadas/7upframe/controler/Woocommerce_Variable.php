...
if(!class_exists('S7upf_Woocommerce_Attributes')){
    class S7upf_Woocommerce_Attributes{
        ...
        static function s7upf_product_option_terms_attribute( $taxonomy, $index ) {
            $types = array(
                'color' => esc_html__( 'Color', 'skincare' ),
                'image' => esc_html__( 'Image', 'skincare' ),
                'label' => esc_html__( 'Label', 'skincare' ),
            );
            ...
            <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'skincare' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr($index); ?>][]">
                <?php
            ...
            <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'skincare' ); ?></button>
            <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'skincare' ); ?></button>
            <button class="button fr plus tawcvs_add_new_attribute" data-type="<?php echo esc_attr($taxonomy->attribute_type); ?>"><?php esc_html_e( 'Add new', 'skincare' ); ?></button>
        ...
        static function s7upf_swatch_html_attribute( $html, $term, $attr, $args ) {
            ...
            switch ( $attr->attribute_type ) {
                case 'color':
                    ...
                    $html = sprintf(
                        '<span class="swatch swatch-color '.$class_white.' swatch-%s %s" '.s7upf_add_html_attr('background-color:'.$color).' data-value="%s"><span class="attr-title">%s</span></span>',
                        esc_attr( $term->slug ),
                        $selected,
                        esc_attr( $term->slug ),
                        $name

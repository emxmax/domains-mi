...
if(class_exists("woocommerce")){
	...
	add_action( 'woocommerce_single_product_summary','s7upf_timer_countdown',11 );
	...
	add_action( 'woocommerce_single_product_summary', 's7upf_get_product_stock', 14 );
	add_action( 'woocommerce_single_product_summary', 's7upf_list_brand_product', 13 );
    ...
    if(!function_exists('s7upf_custom_short_description')){
		function s7upf_custom_short_description( $des ) {
			$des = str_replace('<p>', '<p class="product-desc">', $des);
    ...
    if(!function_exists('s7upf_shop_wrap_before')){
        function s7upf_shop_wrap_before(){
            ...
            ?>
            <div class="product-<?php echo esc_attr($style)?>-view product-<?php echo esc_attr($style)?>-view-<?php echo esc_attr($column);?>-column <?php echo esc_attr($grid_type.' '.$gap_product)?> products-wrap js-content-wrap content-wrap-shop" data-load="<?php echo esc_attr($data_ajax)?>">
    ...
    if(!function_exists('s7upf_product_quickview')){
        function s7upf_product_quickview($icon = '<i class="title24 la la-search-plus"></i>',$text='',$class = '',$echo = true) {
            $html = '<a data-product-id="'.get_the_id().'" href="'.esc_url(get_the_permalink()).'" class="product-quick-view quickview-link '.esc_attr($class).'">'.$icon.$text.'</a>';
            // gg
            // if($echo) echo apply_filters( 's7upf_quickview',$html);
            // else
            return apply_filters( 's7upf_quickview',$html);
        }
    }
    if(!function_exists('s7upf_product_label')){
        function s7upf_product_label($echo = true) {
            ...
            if($uppsell < 0) $html .=   '<span class="new">'.esc_html__("new","skincare").'</span>';
    ...
    if ( !function_exists( 's7upf_catalog_ordering' ) ) {
        function s7upf_catalog_ordering($query,$set_orderby = '',$list_item = false,$add_class = '') {
            ...
            $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
                'menu_order' => esc_html__( 'Default', 'skincare' ),
                'popularity' => esc_html__( 'Sort by popularity', 'skincare' ),
                'rating'     => esc_html__( 'Sort by average rating', 'skincare' ),
                'date'       => esc_html__( 'Sort by newness', 'skincare' ),
                'price'      => esc_html__( 'Sort by price: low to high', 'skincare' ),
                'price-desc' => esc_html__( 'Sort by price: high to low', 'skincare' )
            ) );
            ...
            if ( wc_get_loop_prop( 'is_search' ) ) {
                $catalog_orderby_options = array_merge( array( 'relevance' => esc_html__( 'Relevance', 'skincare' ) ), $catalog_orderby_options );
    ...
    if(!function_exists('s7upf_load_shop')){
        function s7upf_load_shop() {
            ...
            if(!empty($brands)) {
                if(is_string($brands)) $brands = explode(",",$brands);
                $attr_taxquery[]=array(
                    'taxonomy'=>'tax_brand',
                    'field'=>'slug',
                    'terms'=> $brands
        ...
        function s7upf_load_product_filter() {
            ...
            if(isset($pagination) && !empty($pagination)){?>
                <div class="products row list-product-wrap js-content-main">
                    ...
                    else echo '<div class="filter-noresult-wrap"><div class="filter-noresult title18 text-center">'.esc_html__("No result found with current filter value.","skincare").'</div></div>';
                    ?>
                </div>
                <?php
                if($pagination == 'load-more' && $max_page > 1){
                    ...
                    echo    '<div class="btn-loadmore">
                                <a href="javascript:void(0)" class="product-loadmore loadmore"
                                    data-load="'.esc_attr($data_loadjs).'" data-paged="1"
                                    data-maxpage="'.esc_attr($max_page).'">
                                    '.esc_html__("Load more","skincare").'
                                </a>
...
if(!function_exists('s7upf_customize_product_sorting')){
	function s7upf_customize_product_sorting($sorting_options){
		$sorting_options = array(
			'menu_order' => esc_html__( 'Seleccione', 'skincare' ),
			'popularity' => esc_html__( 'Populares', 'skincare' ),
			// 'rating'     => esc_html__( 'Average Rating', 'skincare' ),
			'date'       => esc_html__( 'Novedades', 'skincare' ),
			'price'      => esc_html__( 'Precio: Menos a más', 'skincare' ),
			'price-desc' => esc_html__( 'Precio: Más a menos', 'skincare' ),
		);
...
//Product Stock
if(!function_exists('s7upf_get_product_stock')){
	function s7upf_get_product_stock(){
		global $product;
		...
		if(is_array($stock)){
            ...
            if(!empty($stock['availability'])) $stock = $stock['availability'];
                ...
                if($stock['class'] == 'in-stock') $stock = esc_html__("En stock","skincare");
                else $stock = esc_html__("Agotado","skincare");
            }
        }
		$stock_html =   '<div class="product-available">
							<span class="gray">'.esc_html__("Disponibilidad:","skincare").' </span>
							<span class="'.esc_attr($s_class).'">'.esc_html($stock).'</span>
						</div>';
		echo apply_filters('s7upf_output_content',$stock_html);
	}
}
//Get List Brand
if(!function_exists('s7upf_list_brand_product')){
    function s7upf_list_brand_product(){
		$brands = wp_get_post_terms( get_the_id(), 'tax_brand',array('fields' => 'ids'));
		if(is_array($brands) && !empty($brands)){
			echo '<div class="list-brand-product flex-wrapper align_items-center">
					<label class="title-tax gray">'.esc_html__('Marca:','skincare').'</label>
					<ul class="list-inline-block">';
			foreach($brands as $brand){
				$image_id = get_term_meta ( $brand, 'tax_brand-image-id', true );
				echo '<li><a href="'.get_term_link($brand,'tax_brand').'" class="wobble-horizontal">'.wp_get_attachment_image ( $image_id, "full" ).'</a></li>';
			}
			echo 	'</ul>
				</div>';
		}
...
// Rename the additional information tab encremadascambios
...
function s7upf_woo_rename_tabs( $tabs ) {
	if ( isset( $tabs['additional_information']['title'])){
		$tabs['additional_information']['title'] = esc_html__( 'Información','skincare' );
	}
	return $tabs;
}

...
if(!class_exists('S7upf_BaseController')){
    class S7upf_BaseController{
        ...
        static function _add_scripts(){
		    ...
		    wp_enqueue_script( 'jquery-owl-carousel',$js_url.'lib/owl.carousel.min.js',array('jquery'),null,true);
            ...
            wp_enqueue_script( 'jquery-arctext',$js_url.'lib/jquery.arctext.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'jquery-wow',$js_url.'lib/wow.min.js',array('jquery'),null,true);
            ...
            //wp_enqueue_script( 'smooth-scroll',$js_url.'lib/smooth.scroll.min.js',array('jquery'),null,true);
                ...
                foreach($add_sidebars as $sidebar){
                    if(!empty($sidebar['title'])){
                        ...
                        $custom_add_sidebar = array(
                                'name' => $sidebar['title'],
                                'id' => $id,
                                'description' => esc_html__( 'SideBar created by add sidebar in theme options.', 'skincare'),
        ...
        static function s7upf_product_cat_metabox_add($tag) { 
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content Before','skincare'); ?></label>
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content After','skincare'); ?></label>
                <div class="wrap-metabox">
        ...
        static function s7upf_append_content_before(){
			...
			elseif(is_singular('post')){
				...
				if(empty($page_id)){
					$page_id = s7upf_get_option('before_append');
				}
			}
        ...
        if($s7upf_woo_widgets != 'true' && isset($active_widgets['woocommerce-sidebar']) && empty($active_widgets['woocommerce-sidebar'])){
            ...
            $widgets = array(
                'woocommerce_product_categories' => array(
                    'title' => esc_html__('Product categories','skincare'),
                    'orderby' => 'name',
                    'dropdown' => 0,
                    'count' => 0,
                    'hierarchical' => 1,
                ...
                'woocommerce_price_filter' => array(
                    'title' => esc_html__('Filter by price','skincare'),
                    ),
                'woocommerce_products' => array(
                    'title' => esc_html__('Products','skincare'),
                    'number' =>  5,
                    'show' => '',
                    'orderby' => 'date',
                    'order' => 'desc',

...
if(!function_exists('s7upf_list_post_type')){
    function s7upf_list_post_type($post_type = 'page',$type = true){
        ...
        if($type){
            $page_list[] = array(
                'value' => '',
                'label' => esc_html__('-- Choose One --','skincare')
            );
        }
        else $page_list[] = esc_html__('-- Choose One --','skincare');
        if(is_admin()){
...
if(!function_exists('s7upf_get_order_list')){
    function s7upf_get_order_list($current=false,$extra=array(),$return='array'){
        $default = array(
            esc_html__('None','skincare')               => 'none',
            esc_html__('ID','skincare')                 => 'ID',
            esc_html__('Author','skincare')             => 'author',
            esc_html__('Title','skincare')              => 'title',
            esc_html__('Name','skincare')               => 'name',
            esc_html__('Date','skincare')               => 'date',
            esc_html__('Last Modified Date','skincare') => 'modified',
            esc_html__('Post Parent','skincare')        => 'parent',
            esc_html__('Random','skincare')             => 'rand',
        );
...
if(!function_exists('s7upf_breadcrumb')){
    function s7upf_breadcrumb($step = '') {
        global $post;
        if(is_home() && !is_front_page()) echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Inicio','skincare').'</a>'.$step.'<span>'.esc_html__('Blog','skincare').'</span>';
        else echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Inicio','skincare').'</a>';
        if (is_single()){
        ...
        elseif(is_search()) echo "<span>".esc_html__("Search Results for: ","skincare").get_search_query().'</span>';
        elseif(is_404()) echo "<span>".esc_html__("404 ","skincare")."</span>";
    }
}
//Don't Show popup
        ...
        switch ($type) {
            case 'grid':
                ...
                <ul class="list-inline-block post-meta-data">
                    <li><span class="silver"><?php echo esc_html__('By: ','skincare');?></span><a class="silver" href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>"><?php echo get_the_author(); ?></a></li>
                    <li>
						<span class="silver"><?php echo get_comments_number(); ?></span>
                        <a class="silver" href="<?php echo esc_url( get_comments_link() ); ?>">
                        <?php
                            if(get_comments_number() != 1) esc_html_e(' Comments', 'skincare') ;
                            else esc_html_e(' Comment', 'skincare') ;
                        ?>
                        </a>
                    </li>
                </ul>
                ...
                <ul class="list-inline-block post-meta-data">
                    ...
                    <?php
                        ...
                        <li><i class="fa fa-tags gray" aria-hidden="true"></i>
                            ...
                            <?php if($tags) echo apply_filters('s7upf_output_content',$tags); else esc_html_e("No Tag",'skincare');?>
                        </li>
                    <?php endif;?>
                </ul>
                <?php
...
if(!function_exists('s7upf_get_main_class')){
    function s7upf_get_main_class(){
        ...
        if($sidebar_pos != 'no' && is_active_sidebar( $sidebar['id'])) $main_class = 'content-wrap content-sidebar-'.$sidebar_pos.' col-md-9 col-sm-9 col-xs-12';
...
if ( ! function_exists( 's7upf_get_google_link' ) ) {
    function s7upf_get_google_link() {
        ...
        $fonts  = array(
                    ...
                    'Lora:400,400i,500,700,700i',
                );
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'skincare' ) ) {
...
if(!function_exists('s7upf_list_taxonomy'))
{
    ...
    {
        if($show_all) $list = array( esc_html__('--Select--','skincare') => '' );
        else $list = array();
...
if(!function_exists('s7upf_get_post_style')){
    function s7upf_get_post_style($style = 'element'){
        $list = apply_filters('s7upf_post_item_style',array(
            ...
            esc_html__('Post Flex','skincare')          => 'flex',
            esc_html__('Post Inner','skincare')         => 'inner',
            esc_html__('Post Banner','skincare')        => 'banner',
            esc_html__('Post Banner2','skincare')       => 'banner2',
            esc_html__('Post Banner3','skincare')       => 'banner3',
...
if(!function_exists('s7upf_get_post_list_style')){
    function s7upf_get_post_list_style($style = 'element'){
        $list = apply_filters('s7upf_post_list_item_style',array(
            esc_html__('Default','skincare')      => '',
            esc_html__('Post list 2','skincare')      => 'style2',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_product_style')){
    function s7upf_get_product_style($style = 'element'){
        $list = apply_filters('s7upf_product_item_style',array(
            esc_html__('Default','skincare')      => '',
            esc_html__('Circle','skincare')      => 'circle',
            esc_html__('Quick View','skincare')      => 'quickview',
            esc_html__('Table','skincare')      => 'table',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_product_thumb_animation')){
    function s7upf_get_product_thumb_animation($style = 'element'){
        $list = apply_filters('s7upf_product_item_style',array(
            ...
            esc_html__('Zoom','skincare')        => 'zoom-thumb',
            esc_html__('Rotate','skincare')      => 'rotate-thumb',
            esc_html__('Zoom Out','skincare')    => 'zoomout-thumb',
            esc_html__('Translate','skincare')   => 'translate-thumb',
            esc_html__('Fade','skincare')   => 'fade-thumb',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_list_all_page')){
    function s7upf_list_all_page($complete = false){
        ...
        if(!$complete){
            $page_list = array(
                esc_html__('-- Choose One --','skincare') => '',
                );
        }
                ...
                switch ($preload_style) {
                    ...
                    case 'custom-image':
                        ...
                        <div id="loading-center-absolute-image">
                            <img src="<?php echo esc_url($preload_img)?>" alt="<?php esc_attr_e("preload-image","skincare");?>"/>
                        </div>
                        <?php
                        break;
...
if(!function_exists('s7upf_compare_url')){
    function s7upf_compare_url($icon='',$id = false,$text = '',$class='silver'){
        $html = '';
        if(!empty($text)) $text = '<span>'.esc_html__("Compare","skincare").'</span>';
        ...
        if(class_exists('YITH_Woocompare')){
            ...
            $html = '<a href="'.esc_url($cp_link).'" class="product-compare compare compare-link '.esc_attr($class).'" data-product_id="'.get_the_ID().'">'.$icon.''.$text.'</a>';
        }
        return $html;
    }
}
if(!function_exists('s7upf_wishlist_url')){
    function s7upf_wishlist_url($icon='',$text='',$class='silver'){
        $html = '';
        if(!empty($text)) $text = '<span>'.esc_html__("Wishlist","skincare").'</span>';
        if(empty($icon)) $icon = '<i class="fa fa-heart" aria-hidden="true"></i>';
        if(class_exists('YITH_WCWL_Init')) $html = '<a href="'.esc_url(str_replace('&', '&amp;',add_query_arg( 'add_to_wishlist', get_the_ID() ))).'" class="add_to_wishlist wishlist-link '.esc_attr($class).'" rel="nofollow" data-product-id="'.get_the_ID().'" data-product-title="'.esc_attr(get_the_title()).'">'.$icon.''.$text.'</a>';
        return $html;
    }
}
    ...
    function s7upf_login_form($redirect_to = ''){
        ...
        <div class="login-form popup-form active">
            <div class="form-header">
                <h2><?php esc_html_e( 'Log In','skincare' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Become a part of our community!','skincare' ); ?></div>
                <div class="message ms-done ms-default"><?php esc_html_e( 'Registration complete. Please check your email.','skincare' ); ?></div>
            </div>
            <form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>" method="post">
                <?php do_action( 'woocommerce_login_form_start' ); ?>
                <div class="form-field">
                    <input type="text" name="log" id="user_login" class="input" size="20" autocomplete="off"/>
                    <label for="user_login"><?php esc_html_e( 'Username or Email Address','skincare' ); ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <div class="form-field">
                    <input type="password" name="pwd" id="user_pass" class="input" value="" size="20" autocomplete="off"/>
                    <label for="user_pass"><?php esc_html_e( 'Password','skincare' ); ?></label>
                    <div class="input-focus-line"></div>
                </div>
                ...
                <div class="forgetmenot">
                    ...
                    <label class="rememberme" for="remembermep"><?php esc_html_e( 'Remember Me','skincare' ); ?></label>
                </div>
                <div class="submit">
                    <input type="submit" name="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Log In','skincare'); ?>" />
            ...
            <div class="nav-form">
                <?php if ( get_option( 'users_can_register' ) ) :
                    echo '<a href="#registerform" class="popup-redirect register-link">'.esc_html__("Register","skincare").'</a>';
                endif;
                echo '<a href="#lostpasswordform" class="popup-redirect lostpass-link">'.esc_html__("Lost your password?","skincare").'</a>';
                ?>
            </div>
        </div>
        <?php
...
if(!function_exists('s7upf_register_form')){
    function s7upf_register_form($redirect_to = ''){
        ...
        <div class="register-form popup-form">
            <div class="form-header">
                <h2><?php esc_html_e( 'Create an account','skincare' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Welcome! Register for an account','skincare' ); ?></div>
                <div class="message login_error ms-error ms-default"><?php esc_html_e( 'The user name or email address is not correct.','skincare' ); ?></div>
            ...
            <form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
                ...
                <div class="form-field">
                    ...
                    <label for="user_login"><?php esc_html_e('Username','skincare') ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <div class="form-field">
                    <input type="email" name="user_email" id="user_email" class="input" value="" size="25" autocomplete="off"/>
                    <label for="user_email"><?php esc_html_e('Email','skincare') ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <?php if ( 'no' === get_option( 'woocommerce_registration_generate_password' ) ){ ?>
                    <div class="form-field">
                        <input type="password" name="password" id="reg_passwordp" autocomplete="new-password" />
                        <label for="reg_passwordp"><?php esc_html_e( 'Password', 'skincare' ); ?></label>
                        <div class="input-focus-line"></div>
                    </div>
                <?php }?>
                ...
                <?php if ( 'no' != get_option( 'woocommerce_registration_generate_password' ) ){ ?>
                    <div id="reg_passmail">
                        <?php esc_html_e( 'Registration confirmation will be emailed to you.','skincare' ); ?>
                    </div>
                <?php }?>
                <div class="submit"><input type="submit" name="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Register',"skincare"); ?>" /></div>
                <?php do_action( 'woocommerce_register_form_end' ); ?>
            </form>

            <div class="nav-form">
                <a href="#loginform" class="popup-redirect login-link"><?php esc_html_e( 'Log in','skincare' ); ?></a>
                <a href="#lostpasswordform" class="popup-redirect lostpass-link"><?php esc_html_e( 'Lost your password?','skincare' ); ?></a>
            </div>
        </div>
        <?php
    }
...
if(!function_exists('s7upf_lostpass_form')){
    function s7upf_lostpass_form($redirect_to = ''){
        ...
        <div class="lostpass-form popup-form">
            <div class="form-header">
                <h2><?php esc_html_e( 'Reset password','skincare' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Recover your password','skincare' ); ?></div>
                <div class="message ms-default ms-done"><?php esc_html_e( 'Password reset email has been sent.','skincare' ); ?></div>
                <div class="message login_error ms-error ms-default"><?php esc_html_e( 'The email could not be sent.
Possible reason: your host may have disabled the mail function.','skincare' ); ?></div>
            </div>
            <form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
                ...
                <div class="submit"><input type="submit" name="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Get New Password','skincare'); ?>" /></div>
                <div class="desc note"><?php esc_html_e( 'A password will be e-mailed to you.','skincare' ); ?></div>
            </form>

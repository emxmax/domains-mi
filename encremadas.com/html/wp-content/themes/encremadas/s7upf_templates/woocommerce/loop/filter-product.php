...
<div class="filter-product <?php echo esc_attr($filter_style.' '.$filter_column.' '.$filter_pos)?>" data-load="<?php echo esc_attr($data_loadjs);?>" >
    <a href="javascript:void(0)" class="btn-filter"><span class="color"><i class="fa fa-search"></i></span><?php esc_html_e("Filter","skincare");?></a>
    <div class="box-attr-filter">
        ...
        ?>
            <div class="item-box-attr">
                <div class="item-attr-title">
                    <label><?php esc_html_e("Categories","skincare");?></label>
                </div>
                <div class="item-attr-content">
                    <ul class="list-filter attr-category list-inline">
                        ...
                        foreach ($filter_cats as $cat) {
                            ...
                            if(is_object($term) && !empty($term)) echo '<li><a href="javascript:void(0)" class="element-filter" data-cat="'.esc_attr($cat).'">'.$term->name.'</a></li>';
                        }
                        ?>
                    </ul>
                </div>
        ...
        if($filter_price == 'yes'){
            ...
            <div class="item-box-attr">
                <div class="item-attr-title">
                    <label><?php esc_html_e("Price","skincare");?></label>
                </div>
                <div class="item-attr-content range-filter">
                    ...
                    <div class="attr-price-filter">
                        <p>
                            <label><?php esc_html_e("Filter By Price:","skincare");?></label>
                                        ...
                                        foreach ($terms as $term){
                                            if(is_object($term)){
                                                ...
                                                if(!empty($color)){
                                                    ...
                                                    echo sprintf(
                                                        '<a data-attribute="%s" data-term="%s" class="element-filter swatch swatch-color '.$class_white_color.' swatch-%s %s" '.s7upf_add_html_attr('background-color:'.$color).' title="%s" href="%s"><span class="hide">%s</span></a>',
                                                        esc_attr($attr_filter),
                                                        esc_attr( $term->slug ),
                                                        esc_attr( $term->slug ),
                                                        $selected,

			...
			<form class="search-form live-search-<?php echo esc_attr($live_search)?>" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<?php if($show_cat !== 'off' && !empty($search_in)):?>
					<div class="dropdown-box">
						<span class="dropdown-link current-search-cat"><?php esc_html_e("Todas las categorías","skincare")?></span>
						<ul class="list-none dropdown-list">
							<li class="active"><a class="select-cat-search" href="javascript:void(0)" data-filter=""><?php esc_html_e("Todas las categorías",'skincare')?></a></li>
							<?php
								$taxonomy = 'category';
								...
								if(empty($cats)){
									...
									foreach ($custom_list as $key => $cat) {
										...
										if(!empty($term) && is_object($term)){
											if(!empty($term) && is_object($term)){
												echo '<li><a class="select-cat-search" href="javascript:void(0)" data-filter=".'.$term->slug.'">'.$term->name.'</a></li>';
											}
										}
									}
								}
									...
									if(is_array($product_cat_list) && !empty($product_cat_list)){
										foreach ($product_cat_list as $cat) {
											if($cat->parent == 0 && $cat->slug != 'sin-categorizar'){
												echo '<li><a class="select-cat-search" href="javascript:void(0)" data-filter=".'.$cat->slug.'">'.$cat->name.'</a></li>';
											}
										}
									}
								}
				...
				<div class="list-product-search">
					<p class="text-center"><?php esc_html_e("Por favor, introduzca palabra para mostrar los resultados.","skincare")?></p>
				</div>
			</form>
		</div>
	</div>

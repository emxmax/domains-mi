    ...
    <form class="search-form <?php echo esc_attr($el_class)?> live-search-<?php echo esc_attr($live_search)?>" action="<?php echo esc_url( home_url( '/' ) ); ?>">
        <?php if($show_cat !== 'off' && !empty($search_in)):?>
            <div class="dropdown-box">
                <span class="dropdown-link current-search-cat"><?php esc_html_e("All Categories","skincare")?></span>
                <ul class="list-none dropdown-list">
                    <li class="active"><a class="select-cat-search" href="javascript:void(0)" data-filter=""><?php esc_html_e("All Categories",'skincare')?></a></li>
                    <?php
                        $taxonomy = 'category';
        ...
        <div class="list-product-search">
            <p class="text-center"><?php esc_html_e("Please enter key search to display results.","skincare")?></p>
        </div>
    </form>
</div>
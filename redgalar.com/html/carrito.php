    ...
    <div class="container">
    	<?php
    		if (isset($_POST['submit'])) {
				...
				$per_referencia = $_POST['per_referencia'];
				$telf_contacto = $_POST['telf_contacto'];
				$mi_telf = $_POST['mi_telf'];
				$persona_contacto = $_POST['persona_contacto'];
				// $comentarios_conduc = $_POST['comentarios_conduc'];
			}
    	?>
	<form role="form" method="POST" id="add_comprar_pro">
      <div class="row">
          ...
          <div class="list-carrito">
			...
			//consultamos los productos conforme a la empresa y el usuario
				...
				?>
				  ...
				  <div class="col-lg-7 col-md-7 col-sm-7 box-envio-<?php echo $nn?>">
					<div class="row">
					  <div class="col-lg-6 col-md-6 col-sm-7 frm_mas_data">
						<input type="text" class="form-control datepicker fecha" placeholder="*Fecha de envio" name="fecha" id="fecha<?php echo $nn?>" required>
					  </div>
					  <div class="col-lg-6 col-md-6 col-sm-6 frm_mas_data">
						<select name="hora" id="hora<?php echo $nn?>" required class="form-control hora" required>
					  ...
					  <?php
						...
						WHERE precio_empresa.id_emp = ".$fila["id_empresa"] ." and dis_name<>'Recojo' 
						ORDER BY distrito.dis_name ASC";
						$distrito = mysql_query($sql2);

						$sql3 = "SELECT * FROM distrito WHERE dis_name='Recojo'";
						$rsUsuario = mysql_query($sql3);
						$precio1 = mysql_result($rsUsuario,0,"dis_precio");
						$name1 = mysql_result($rsUsuario,0,"dis_name");
						$dis_id = mysql_result($rsUsuario,0,"dis_id");				

					  ?>
						<select class="form-control distrito_val distrito" id="distrito<?php echo $nn?>" name="distrito" required>
							<option value="" data-emp="<?php echo $fila["id_empresa"];?>">Distrito</option>
							<option value="<?php echo $precio1;?>" data-id="<?php echo $dis_id; ?>" data-emp="<?php echo $fila["id_empresa"] ?>"><?php echo utf8_encode($name1);?> - S/.<?php echo $precio1;?></option>
							<?php
					  ...
					  <div class="col-lg-6 col-md-6 col-sm-6 frm_mas_data">
						<input type="email" class="form-control email" id="mi_email<?php echo $nn?>"   placeholder="Mi email" name="email" required>
					  </div>
					<?php
					if ($user_direccion1 != '') {
						?>
						<div class="btn-group col-lg-12 col-md-12 col-sm-12 frm_mas_data" data-toggle="buttons">
							<label class="btn btn-primary">
								<input type="checkbox" name="direccion" class="direccion_1" value="<?php echo $user_direccion1;?>" autocomplete="off">&nbsp;<?php echo $user_direccion1; ?>
							</label>
						</div>
						...
						<div class="btn-group col-lg-12 col-md-12 col-sm-12 frm_mas_data" data-toggle="buttons">
							<label class="btn btn-primary">
								<input type="checkbox" name="direccion" class="direccion_2" value="<?php echo $user_direccion2;?>" autocomplete="off">&nbsp;<?php echo $user_direccion2; ?>
							</label>
						</div>
						<?php
					}
					  ?>
					  <div class="col-lg-12 col-md-12 col-sm-12 frm_mas_data">
						<textarea class="form-control direccion" id="direccion<?php echo $nn?>"  name="direccion" placeholder="Ingrese dirección" required></textarea>
					  </div>
					  <div class="col-lg-12 col-md-12 col-sm-12 frm_mas_data">
						<textarea class="form-control per_referencia" id="per_referencia<?php echo $nn?>"  placeholder="Ingrese la referencia" name="per_referencia" required></textarea>
					  </div>
					  <?php
						if($nn > 1){
						echo '<div class="col-lg-6 col-md-6 col-sm-6 frm_mas_data">
								<div class="checkbox">
								<label>
					  ...
					  <div class="col-lg-6 col-md-6 col-sm-6 frm_mas_data">
						<input type="text" class="form-control telf_contacto" id="telf_contacto<?php echo $nn?>"  placeholder="Telf. del destinatario" name="telf_contacto" required>
					  </div>
					  <div class="col-lg-6 col-md-6 col-sm-6 frm_mas_data">
						<input type="text" class="form-control mi_telf" id="mi_telf<?php echo $nn?>"   placeholder="Mi teléfono" name="mi_telf" required>
					  </div>
					  <div class="col-lg-12 col-md-12 col-sm-12 frm_mas_data">
						<input type="text" class="form-control persona_contacto" id="persona_contacto<?php echo $nn?>"  placeholder="Persona de contacto" name="persona_contacto" required>
					  </div>
					  <div class="col-lg-12 col-md-12 col-sm-12 frm_mas_data">
						<textarea type="text" class="form-control comentarios_conduc" id="comentarios_conduc<?php echo $nn?>"  name="comentarios_conduc" placeholder="Comentarios al Courier"></textarea>
					  </div>
					  <?php
						if($nn > 1){
						echo "</div>";
        ...
        <div class="col-lg-4 col-md-4 col-sm-12">
		  <div class="comprobante">
			<div class="row">
				...
				<div id="doc_nume" class="col-md-12 col-sm-12">
					<label>DNI:</label>
					<input type="text"  maxlength="8" class="form-control" name="doc_num" id="doc_num" required> <!-- minlength="8" -->
				</div>
				<div id="doc_nomb" class="col-md-12 col-sm-12">
					<label>Nombre:</label>
					<input type="text" class="form-control" name="doc_nom" id="doc_nom" required>
				</div>
				<div class="col-md-12 col-sm-12" id="doc_direc" style="display:none">
          ...
          <div class="resumen">
            <ul>
              ...
              <li>
				<button type="submit" class="btn-100 btn-complete" name="submit">PROCESAR COMPRA</button>
                <!--<a id="comprar" href="#">PROCESAR COMPRA</a>-->
                <?php
                	include("validar_carrito.php");
                ?>
              </li>
            </ul>
          </div>
...
<div id="alerta_carrito"></div>
 ...
 //  $('#add_comprar_pro').submit(function(e){
  ...
  <script>
	...
	<script>
		$(document).ready(function(){
			$(".direccion_1").click(function() {
				var claseActiva = $('.direccion_2').hasClass('active');
				console.log('Direccion2 : ' + claseActiva);
				if (claseActiva==true) {
					console.log("entro al primero se eliminar el segundo");
					$('.direccion_2').removeAttr('checked');
					$(".direccion_2").removeClass('active');
				}
				if( $(this).is(':checked') ){
					$("textarea.direccion").attr('disabled','disabled');
					$(this).addClass('active');
			} else {
					$("textarea.direccion").removeAttr('disabled');
					$(".direccion_2").parent().show();
					$(this).removeClass('active');
			}
			});
			$(".direccion_2").click(function() {
				var claseActiva = $('.direccion_1').hasClass('active');
				console.log('Direccion1 : ' + claseActiva);
				if (claseActiva==true) {
					$('.direccion_1').removeAttr('checked');
					$(".direccion_1").removeClass('active');
				}
				if ($(this).is(':checked')) {
					$("textarea.direccion").attr('disabled','disabled');
					$(this).addClass('active');
				} else{
					$("textarea.direccion").removeAttr('disabled'); //elimima el checked del checkbox
					$(".direccion_1").parent().show();
					$(this).removeClass('active');
				}
			});
		});

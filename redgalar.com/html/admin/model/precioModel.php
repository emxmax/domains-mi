...
class Precio extends Conectar
{
	...
	//lista de listpreicos x distrito
	...
	public function newprecio(){
		...
		if(mysql_num_rows($res1) < 1){
			...
			//insertar usuario
			$sql = "insert into precio_empresa values(null,'".$d."','".$p."','".$i."')";
			//print_r ($sql );
			$res = mysql_query($sql,Conectar::con());
			if(isset($res)){
				echo "exito";
	...
	public function deleteprecio(){
		$id = $_POST["id"];
		//insertar tarea
		$sql = "delete from precio_empresa 		
			WHERE id =".$id;
		$res = mysql_query($sql,Conectar::con());
		if(isset($res)){
    		echo "exito";
	...
	//Activar el udateprecio
	public function udateprecio(){
		$id = $_POST["id_p"];
		$pr = $_POST["precio"];
		$ide = $_POST["distrito"];
		
		//actualiza precios
		$sql = "UPDATE precio_empresa 
			SET precio = '".$pr."'
			WHERE id =".$id;
		$res = mysql_query($sql,Conectar::con());
		if(isset($res)){
	...
	public function editproducto(){
		...
		$pro_fecha_entrega = $_POST["pro_fecha_entrega"];
		...
		if($tipo =="image/pjpeg" OR $tipo =="image/jpeg" OR $tipo =="image/png"){
			...
			cate_id = '$cate_id', 
			pro_codigo='$c',
			pro_fecha_entrega='$pro_fecha_entrega'
			WHERE pro_id =".$id;
		}
		else{
			$sql = "UPDATE producto 

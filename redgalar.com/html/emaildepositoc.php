<?php
	...
	//id del pedido
	$sqlId = "SELECT * FROM pedido WHERE pe_id='$iddd'";
	$rsId= mysql_query($sqlId, $cn);
	$pe_id = mysql_result($rsId,0,"id_codigo");
	
	//echo $pe_id;
	//$pe_id = "1-00000011";

	$sqlUsu = "SELECT * FROM detalle_pedido WHERE pe_codigo='$pe_id'";
	$rsUsu= mysql_query($sqlUsu, $cn);
	$iduser = mysql_result($rsUsu,0,"id_user");
	
	//$user_email= $_SESSION['email'];
	//datos para el correo
	$sqlUsuario = "SELECT * FROM usuario WHERE user_id='$iduser'";
	...
	while ($fila = mysql_fetch_array($empresas)) {
		...
		//consultamos los productos conforme a la empresa y el usuario
			...
			while ($list = mysql_fetch_array($productos)) {
				$htmlArt .='<div class="row pedido-carrito" style="display: flex;margin-bottom:2px">
				...
				<div class="row">
				  ...
				  <div class="col-lg-6" style="display: inline-block;width:50%;font: 11px arial;">';
					...
					$htmlArt .= '<p style="margin: 0px;margin-bottom: 5px;"><strong>Distrito:</strong> '.utf8_encode($ee["dis_name"]).'</p>
						  ...
						  <div class="col-lg-6" style="display: inline-block;width:48%;font: 11px arial;">
  ...
  $bodyMsj = '<body>
		...
		<div class="order" style="background: #F0F1F5;margin-top: 10px;border-radius: 4px;">
			<div class="inner-detallado" style="padding-left: 28px; padding-bottom: 10px;">
				<div style="width:63%; display:inline-block;vertical-align: top;">
					<h4 style="color: #D2333D;font: 13px arial;font-weight: 900;margin-bottom:0px">Tu pedido ha sido procesado con éxito</h4>
					<p style="font: 11px arial;color: #666666;margin:0px">Si tienes alguna consulta sobre tu compra nuestro equipo estará dispuesto a ayudarte.</p>
					<p style="font: 11px arial;color: #666666;margin:3px 0px">Nuestros canales de atención son:</p>
					<a href="http://www.redgalar.com" target="_blank" style="font: 11px arial;color: #666666;">www.redgalar.com</a><br>
					<a href="http://www.facebook.com/redgalar" target="_blank" style="font: 11px arial;color: #666666;">facebook.com/redgala</a>
				</div>
				<div style="width:34%; display:inline-block;text-align: center;">
					<img src="https://www.redgalar.com/img/email/fa-gift.png" style="padding-top:15px" alt="img">
				</div>
			</div>
			...
			<div class="inner" style="padding: 0 35px;">
				...
				<div class="core">
					...
					<div class="inner-row" style="display: flex;flex-flow: row wrap;">
						...
						<div class="col" style="width: 42%;margin-left: 10px;">
							<p style="font: 11px arial;color: #666666;font-weight: 400;">Depósito</p>
						</div>
					</div>
				</div>
			</div>
	...
	//$para = "genixx_10@hotmail.com";
	$para = "erick@redgalar.com,$eee";
	...
	$cabeceras .= 'From: Nuevo Pedido Deposito<info@redgalar.com>';
	...
	if ($resultado == true)
	{ 
		//echo "Gracias por rellenar el formulario. Se ha enviado correctamente a la dirección Redgalar "; 
	}
	
  /*END CORREO*/
	$metodopago = "Depósito";
	require "emailproveedor.php";
	
	require "emailredgalar.php";
  //print_r($mensaje);
?>
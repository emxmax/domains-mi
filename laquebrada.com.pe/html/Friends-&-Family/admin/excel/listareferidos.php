...
$sqlClienteAdmin = "SELECT re_id, re_name, re_dir, re_email, re_telf FROM referido";
...
$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A1', "Nombre del Referidos")
...
for ($i=0;$i<=$item;$i++){
	$data 			= $main_arr[$i];
	$name 	        = $data['re_name']; 
	$direccion 		= $data['re_dir']; 
	$email 		    = $data['re_email']; 
	$telefono 		= $data['re_telf']; 
	/*** Llenamos el archivo *****/
	$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A'.$countexcel, $name)
		->setCellValue('B'.$countexcel, $direccion)
		->setCellValue('C'.$countexcel, $email)
		->setCellValue('D'.$countexcel, $telefono);
	$countexcel++;
}
if ((isset($_GET['id'])) and (!empty($_GET['id'])))
	$nameXls = sanear_string($propietario);
else
	$nameXls = "listadoreferenciados";

...
class Noticia extends Conexion{
	function dameCodigo($url=NULL){
		$sql = "Select tar_art_id ";
		$sql.= "From tseo_seo ";
		$sql.= "Where seo_url = '".$url."' ";
		$sql.= "LIMIT 0,1";
		$data = $this->Sqlfetch_assoc($sql);
		return $data[0]['tar_art_id']; 
	}
	function noticiaDestacada($id=NULL,$page=NULL,$TAMANO_PAGINA=NULL,$varLang=NULL){
		$sql = "Select art_id, tca_cat_id, art_nombre, art_nombre_en, art_descripsuperior, art_descripsuperior_en, art_descripinferior, art_descripinferior_en,art_frase, art_frase_en, ";
		$sql.= "(Select tmp_cat.cat_nombre From tca_categoria as tmp_cat Where tmp_cat.cat_id = tca_cat_id Limit 0,1) as namecategoria, ";
		$sql.= "(Select tmp_cat.cat_nombre_en From tca_categoria as tmp_cat Where tmp_cat.cat_id = tca_cat_id Limit 0,1) as namecategoria_en, ";
		...
		if ($varLang=="en"){
			$sql.= "and art_nombre_en != ''  ";
		}
		$sql.= "and tca_cat_id = $id ";
		$sql.= "Order by art_fechacreacion DESC ";
		$sql.= "LIMIT 0,$TAMANO_PAGINA";
	...
	function dameListado($page=NULL,$filtro=NULL,$inicio=NULL,$TAMANO_PAGINA=NULL,$gIdioma=NULL){
		$page = $this->limpiacadena($page);
		$filtro = $this->limpiacadena($filtro);
		$gIdioma = $this->limpiacadena($gIdioma);
	...
	function dameListado2($page=NULL,$filtro=NULL,$inicio=NULL,$TAMANO_PAGINA=NULL,$gIdioma=NULL){
		$page = $this->limpiacadena($page);
		$filtro = $this->limpiacadena($filtro);
		$gIdioma = $this->limpiacadena($gIdioma);
		...
		$sql.= "Order by namecategoria ASC, art_fechacreacion DESC ";
		if ($TAMANO_PAGINA > 0)
			$sql.= "LIMIT ".$inicio."," . $TAMANO_PAGINA;
		return $this->Sqlfetch_assoc($sql);	
	}
	function detalleBibliografia(){
		$codigo = 7;
	...
	function prodeceAregar($data=NULL){
		...
		$sql.= "art_fechapublicacion,art_estado,art_fechacreacion,art_fechamodificacion ) ";
		$sql.= "Values ";
		...
		$sql.= "'$f_publicacion','$estado','$fecha','$fecha' ) ";
		//return $sql;

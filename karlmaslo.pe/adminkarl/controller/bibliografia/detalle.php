...
if (isset($data['codigo'])) {
	if (($data['codigo'] > 0)) {
		...
		for($i=0; $i<=$item; $i++){
			...
			$arrjson[$i]['titulo_en'] 	= $objfunc->convertir_html($data['art_nombre_en']);
			...
			$arrjson[$i]['descrip_superior_en'] = $objfunc->convertir_html($data['art_descripsuperior_en']);
			$arrjson[$i]['descrip_inferior'] = $objfunc->convertir_html($data['art_descripinferior']);
			$arrjson[$i]['descrip_inferior_en'] = $objfunc->convertir_html($data['art_descripinferior_en']);
			$arrjson[$i]['frase'] 			 = $objfunc->convertir_html($data['art_frase']);
			$arrjson[$i]['frase_en'] 		 = $objfunc->convertir_html($data['art_frase_en']);

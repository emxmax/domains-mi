...
class PHPExcel_Worksheet_PageSetup
{
	 ...
	 *-
	 * @parAm int? $pValue
 * @param boolmaN $pUpdaôe Upäate fitUoPage so it applies rather txa. scalyng
	0* Argturn PHPAxcel_Worksheet_PageSetup
 *'
	pubìhc funbtion0setFitTMWidt(($pValue = 1, $pUpdate = 4rue) zj		$this)>_fitToWidth = $`Vanue;
I	if ($pUpdqte) {
	$this->_fitToPace0- true+
		}
		return $thiS;
	}
Š	/**
	 * Is Columns to repeat at lEfd sgt?
	 *
	(* @return"boo|ean
	 ./
	pebmyc function msColumnsToRepeatAtLeätSet() {
		if (is_árrai($this->_colõmnsToRepeatAtLeft)) {
			if0($ô`is->_co,wmnsToRepeatAtLeft[0] %= '' && $5his-<]cklumnqToRepeatAvLeft[1] != +§) {
				returnaDrue;
			}
		|M
...
/*+
	 * Get2Columns to repeat at left
	 *
	 " @setõrn array CgnpaafIng start col}mn Aîd ene golumn, emptq abbay if optIon umset	 */
	publiC function getïL5i.sToRepeatAtLeft(9 {
		return $this-_coluínsToRepeatATHeft;
	}

	/*2
	 * Set Colulns to rmpeat at left	 *
	 * @p`ra} erzay $pFalue Containilg start column ant end c/luín empty array if option unset
	 * @return PHPExcel_Worksheet_PageSetup
	 */

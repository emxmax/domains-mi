function Contenido()
{
	...
	function mostrarContenido(data)
	{
		...
		if(data.titulo != null){
			$("#contenidoPagina").append('<h1>'+data.titulo+'</h1>');
		}		
		...
		if(contenidos.length > 0){
			$.each(contenidos, function( index, value ) {
				...
				if(sbTitt!=null){
					$("#contenidoPagina").append('<div class="row contenido" id="ct'+idC+'">');
				...
				if(formulario != null){
						...
						if(campos!=null){
							...
							$("#ct-formulario"+idFormulario).append('<div class="row botonera"><button class="btn btn-primary" id="btnGrabarForm'+idFormulario+'">Enviar</button><button class="btn btn-info limpiar" id="btnLimpiarForm'+idFormulario+'" data-section="ct-formulario'+idFormulario+'">Limpiar</button></div>');
			...
			$.each(contenidos, function( index, value ) {
				...
				if(index % 2 ==0){
					row=row+1;
					$("#contenidoPagina").append('<div class="row enlacesImg" id="enlaceImg'+row+'">');
	...
	function mostrarNoticias(data)
	{
		...
		$.each(data, function( index, value ) {
			...
			$(".itemNoticia"+idPost+" > .col").append('<a href="'+url+'"><button class="tbn btnExsa btnVerMasNoticia" data-id="'+idPost+'">Leer Más</button></a>');

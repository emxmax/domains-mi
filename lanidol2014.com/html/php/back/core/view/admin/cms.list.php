...
if($oItem->parentContentID>0){
	...
	$bEditSection=($oSection!=null && $oSection->isEditable);
...
<?php 
	$bMoveUp=true;
                    ...
                    <div id="divTemplateList">
                        ...
                        <div class="body">
                        <?php
                        //echo "($parentSchemeID, $oItem->sectionID)";
                        ...
                        if(CmsScheme::getErrorMsg()!="") $MODULE->addError(CmsScheme::getErrorMsg());
                            ...
                            foreach ($lScheme as $obj) {
                                $enabled="";
                                $style="";
								$cnt=CmsContentLang::getCount_ChildScheme($obj->schemeID, $oItem->sectionID, $oItem->langID, $oItem->minisiteID);
                                //if($obj->iterations>0 && $obj->countContent>=$obj->iterations){
                                if($obj->iterations>0 && $cnt>=$obj->iterations){
                                    $enabled="disabled='true'";
                                    $style="class='option-disabled'";
                                }
                                echo "<div class=\"item\">
								<table width='100%'><tr><td width='12' valign='top'><input type=\"radio\" name=\"schemeID\" id=\"scheme_".$obj->schemeID."\" value=\"".$obj->schemeID."\" ".$enabled." /></td><td><label for=\"scheme_".$obj->schemeID."\" ".$style." >".$obj->alias."</label></td></tr></table>
								</div>\n";
                            }
                        ?>
                        </div>

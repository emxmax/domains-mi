...
var $urlContent;
	...
	global $ADMIN;
		...
		$this->UsrSession	=$_SESSION[$ADMIN["SESSION"]];
	...
	function ValidateEvent($command){
		...
		if($this->lUserEvents == null || $this->lUserEvents->getLength()==0){
			$this->addError("No tiene acceso a este módulo, permiso denegado.");
			return false;
		}
	...
	function processFormAction($DAO, $oItem){
		...
		if(!$this->ValidateEvent($this->GetCommandEvent($this->Command))){
			$this->addError("No puede realizar esta acción, permiso denegado.");
			return;
		}

...
class CmsContent extends Database
{
	...
	public static function  getItem($contentID){
		$query = "SELECT contentID, parentContentID, schemeID, minisiteID, contentName, position
				FROM cms_content
        ...
        if ($row = parent::fetchArray($result)) {
            $oItem= new eCmsContent($row['contentID'], $row['parentContentID'], $row['schemeID'], $row['minisiteID'], $row['contentName'], $row['position']);
        }
		return $oItem;
	}
	...
	public static function  getList(){
        ...
        while ($row = parent::fetchArray($result)) {
            $list->addItem(new eCmsContent($row['contentID'], $row['parentContentID'], $row['schemeID'], $row['minisiteID'], $row['contentName'], $row['position']));
        }
	return $list;
	}
	...
	public static function  AddNew($oContent){
		...
		$query = "SELECT COUNT(cont.position) FROM cms_content AS cont
			...
			WHERE IFNULL(cont.parentContentID,0) ='$oContent->parentContentID'  AND schm.sectionID='$oContent->sectionID' AND cont.minisiteID='$oContent->minisiteID'";
		$result = parent::GetResult($query);
		$oContent->position = parent::fetchScalar($result)+1;

		//Insert data to table
		TextParser::ParseEntityQuotes($oContent);
		$query = "INSERT INTO cms_content(contentID, parentContentID, schemeID, minisiteID, contentName, position)
				VALUES('$oContent->contentID', ".($oContent->parentContentID!=null ? $oContent->parentContentID: "null").", '$oContent->schemeID', ".($oContent->minisiteID!=null ? $oContent->minisiteID: "null").", '$oContent->contentName', '$oContent->position')";
		return parent::Execute($query);
	}

	public static function  MoveUp($oContent){
		//Search Previous Id
		$query = "SELECT cont.contentID, cont.parentContentID, cont.schemeID, cont.minisiteID, cont.contentName, cont.position 
		...
		cont.minisiteID = '$oContent->minisiteID' AND
		...
		$result = parent::GetResult($query);
		if($row = parent::fetchArray($result)){
			$oContentPrev =new eCmsContent($row['contentID'], $row['parentContentID'], $row['schemeID'], $row['minisiteID'], $row['contentName'], $row['position']);

...
class CmsContentLang extends Database
{
    ...
    public static function  getItem($contentID, $langID){
        ...
        SELECT 
            ...
            cont.parentContentID, cont.schemeID, cont.minisiteID, cont.contentName, cont.position, sch.sectionID, sch.templateID, sch.isPage, 0 AS childScheme
        FROM 
            cms_content AS cont
    ...
    public static function  getList_Paging($parentContentID, $sectionID, $langID, $minisiteID){
        ...
        SELECT 
            ...
            cont.parentContentID, cont.schemeID, cont.minisiteID, cont.contentName, cont.position, sch.sectionID, sch.templateID, sch.isPage, 0 AS childScheme, 0 AS childContentLang
        FROM 
            cms_content AS cont
        INNER JOIN cms_scheme AS sch
            ON sch.schemeID=cont.schemeID
    ...
    public static function getWebList_Likes($parentContentID, $sectionID, $langID, $minisiteID, $orderBy=NULL, $showInHome=false){
        $query ="
        SELECT 
            cont.contentID, clan.langID, clan.title, clan.subTitle, clan.subTitle2, clan.description, clan.description2, clan.resumen, clan.date, clan.linkType, clan.linkContentID, clan.linkURL, clan.linkTarget, clan.staticURL, clan.media, clan.parameter, clan.metaTag, clan.registerDate, clan.showInHome, clan.active, 
            cont.parentContentID, cont.schemeID, cont.minisiteID, cont.contentName, cont.position, sch.sectionID, sch.templateID, sch.isPage, (SELECT COUNT(*) FROM cms_content_likes WHERE contentID=cont.contentID) AS totalLikes
        FROM 
            cms_content AS cont
        INNER JOIN cms_scheme AS sch
            ON sch.schemeID=cont.schemeID
    ...
    public static function  getCount_ChildScheme($schemeID, $sectionID, $langID, $minisiteID){
        $query ="
                ...
                WHERE
                        IFNULL(cont.schemeID,0)         = '$schemeID'
                    AND IFNULL(schm.sectionID, 0)       = '$sectionID'
                    AND IFNULL(clan.langID, 0)          = '$langID'
                    AND IFNULL(cont.minisiteID, 0)      = '$minisiteID'
                ";

...
class CmsParameterLang extends Database
{
	...
	public static function  getList($groupID, $langID){
		$query = "SELECT param.parameterID, param.groupID, plan.langID, IF(plan.parameterName IS null, param.alias, plan.parameterName), param.position, plan.description, plan.attribute, plan.active, IF(plan.parameterID IS null, 1, 0) AS nullValue
		FROM cms_parameter AS param
		LEFT JOIN cms_parameter_lang AS plan
			ON plan.parameterID=param.parameterID
			AND IFNULL(plan.langID, 0) = '$langID'
	...
	public static function  AddNew_ParameterLang($oParameterLang){
		...
		$query = "INSERT INTO cms_parameter_lang(parameterID, langID, parameterName, description, attribute, active)
				VALUES('$oParameterLang->parameterID', '$oParameterLang->langID', '$oParameterLang->parameterName', '$oParameterLang->description', '".XMLParser::parseXML_Parameter($oParameter->attribute)."', '$oParameterLang->active')";

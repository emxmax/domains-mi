...
//CLASES GENERALES
    public function guardarImagen($imagen,$directorio,$nombre){
        $Base64Img=$imagen;
    ...
    **/
    ...
    public function enviarCorreo($data)
    {
        ...
        $message = '<html>
                    ...
                    for($i=0;$i<$cantidad;$i++){
                        $message.=' <tr>
                                        ...
                                        <td colspan="2" class="valor" style="color: #3a3a3a;">'.strtoupper($valores[$i]).'</td>
                                    </tr>';
                    }                       
                    $message.=' </table>
                            </div>
        ...
        $resultado=mail('info@exsa.net',"Formulario web - Exsa",$message,$headers);
        if($resultado){
            return 1;
        }else{
            return 0;
    ...
    public function login($data){
        $data = json_decode($data);
        $query ="SELECT idUsuario,nombre FROM usuarios WHERE usuario='$usuario' AND clave='$clave' AND estado=1";

...
class Paginador
{
    ...
    public function setPropagar(Array $variables,$separador='')
    {
        $getUrl = '';
        foreach ($variables as $valor) {
            //$valorUrl= isset($_GET[$valor])? $_GET[$valor] : null;
           	//$getUrl .= urlencode($valor).$separador;
			$getUrl .= $separador.$valor;
        }
        if($separador=='')$getUrl = substr($getUrl, 0, strlen($getUrl)-1);
     ...
     */
    public function getHtmlPaginacion($varGet = 'pagina', $cont = 'li',$destino='',$titulo='')
    {
        $paginador = array();
        foreach ($this->_paginacion as $enlace) {
            ...
            $htmlPag   .= '<a href="' . $this->_urlDestino .$destino 
                        . $varGet.$enlace['numero'] . $this->_propagar.$titulo. '" '
                        . 'title="' . $enlace['title'] . '" '
                        . '>' . $enlace['vista'] . '</a>'; 
            } else {

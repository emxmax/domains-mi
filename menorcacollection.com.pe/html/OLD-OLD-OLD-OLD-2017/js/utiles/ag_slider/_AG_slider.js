(function($) {
	...
	$.AG_slider = function(el, opc) {
		...
		base.init = function(){
			...
			base.totalElementos = $(base.opc.elemento).length;
			...
			$(base.opc.elemento).filter(function(i){base.datos[i] = i+1;});
			
			//alert("total  "+base.elementosRestantes);
			...
			if(base.totalElementos>1){ 
				$(base.opc.elemento).bind('click',function(){
					//alert($(this).index());
			...
			if(base.opc.activarPaginacion==true && base.opc.paginadorPersonalizado==false){
	...
	*/
		...
		base.tituloSlide = function(){
			$(base.opc.titulo).not(":eq("+base.posicion+")").hide();
			$(base.opc.titulo).eq(base.posicion).slideDown();
		}
	
		base.conteoSlide = function() {
			//console.log(base.posicion);
		...
		base.moverSlide = function(){
			...
			console.log("si 5  "+base.contador);
		...
		base.activarDelay = function(moviendo){
			...
			if (moviendo){
				...
				base.delay = window.setInterval(function() {
					...
					if(base.contador>base.aDesplazar && base.aDesplazar!=0){
						base.contador = 0;
						base.moverSlide();
						console.log("si 2  "+base.aDesplazar);
						base.movIzq=0;
						base.movDer=1;
						if(base.opc.botones==true){
							$(base.opc.botonIzq).addClass(base.opc.botonIzqInactivo);

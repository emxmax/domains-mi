(function($) {
	...
	$.ag_slider = function(el, opc) {
		...
		base.init = function(){
			...
			//console.log("si -> "+base.totalElementos);

			if(base.opc.mensajeModo2==true){
				$(base.opc.elementoMensajeModo2).hide();
				$(base.opc.elementoMensajeModo2).eq(0).slideDown(300);
			}

			//console.log("total ->"+base.$el.find('> li').length);
			
			/*base.$el.find('> li .video').each(function(i){
				console.log("elemento -> "+i);
				var id = $(this).attr('id');
				//var muid = base.uid+"_"+i;
				base.loadKMTrackedVideo('uU6U-8LP1DY', 960, 346, 'video');
			});*/
	...
	*/
		...
		base.tituloSlide = function(){
			$(base.opc.titulo).html(base.datos[base.contador]);
		}
	
		base.conteoSlide = function() {
			$(base.opc.conteo).html(base.datos[base.contador] + ' de ' + base.totalElementos);
		}
		...
		base.moverSlide = function(){
			...
			if(base.opc.mensajeModo2==true){
				$(base.opc.elementoMensajeModo2).hide();
				$(base.opc.elementoMensajeModo2).eq(base.contador).slideDown(300);
			}
			...
			if(base.opc.objeto!=null){
				base.opc.objeto.inicio1.deshabilitar(base.contador);
				base.opc.objeto.inicio1.habilitar(base.contador);
				base.opc.objeto.inicio1.eventos();
			}
		...
		base.loadKMTrackedVideo = function(id, width, height, name){
			//console.log("video load");
			var uid = 'player_' + id;
			base.players[id] = new YT.Player( uid, {
				videoId: id,
				width: width,
				height: height,
				playerVars: {
				...
				events: { onStateChange: base.test}
			});

		}

		base.averiguar = function(){
			//console.log("estado del reproductor -> "+base.est.data);
			
			switch(base.est.data){
				case 1:
					return base.est.target.stopVideo();
			}
		}

		base.test = function(event){
			base.est = event;
			alert(base.est);
		}

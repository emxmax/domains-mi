...
function AddLike($postitID){
    ...
    if($usrSession!=NULL){
        ...
        if(CrmPostIt::Validar_AddLike($postitID, $personaID)){
            if (CrmPostIt::AddLike($postitID, $personaID)){
                echo CrmPostIt::getTotalLikes($postitID);

                //Registro de Logs
                $oEmpleadoLog=new eCrmEmpleadoLog();
                $oEmpleadoLog->personaID	=$personaID;
...
function getPostItItem($oItem){
	$postit_view=file_get_contents("../view/website/postit/view_default.html");

...
class AdmProfileEvent extends Database
{
	...
	public static function  getList_UserProfile($profileID){
		if($profileID==1 || $profileID==2){
			$query = "
				SELECT '$profileID' AS profileID, event.eventID, event.moduleID, event.command
				FROM	adm_event AS event";
			//if($profileID==2) //Filter for webmaster admin
			//$query = "
			//	WHERE event.moduleID NOT IN(0)"; //specify Modules that has super admin
		}
		else
			$query = "
				SELECT prof_event.profileID, prof_event.eventID, event.moduleID, event.command
				FROM	adm_event AS event

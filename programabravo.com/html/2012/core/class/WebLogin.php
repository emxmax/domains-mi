...
class WebLogin extends Database
{
	...
	public static function Logon($userName, $password, $rememberMe=false)
	{
		...
		if($oCrmEmpleado!=NULL){
			...
			self::$usrSession["nombres"]        =$oCrmEmpleado->nombres;
			self::$usrSession["apellidos"]      =$oCrmEmpleado->apellido1.' '.$oCrmEmpleado->apellido2;
			self::$usrSession["nombreCompleto"] =$oCrmEmpleado->nombres.' '.$oCrmEmpleado->apellido1.' '.$oCrmEmpleado->apellido2;
			self::$usrSession["email"]          =$oCrmEmpleado->email;
			self::$usrSession["mundo"]          =$oCrmEmpleado->unidadOrganizativa;

...
class CmsParameterGroup extends Database
{

	public static function  getItem($groupID){
		$query = "SELECT * FROM cms_parameter_group 
	...
	public static function  AddNew($oGroup){
		//Search the max Id
		$query = "SELECT MAX(groupID) FROM cms_parameter_group";
		$result = parent::GetResult($query);
		$oGroup->groupID= parent::fetchScalar($result)+1;
		//Insert data to table
		$query = "INSERT INTO cms_parameter_group(groupID, groupName, active)
	...
	public static function  Update($oGroup){
		...
		$query = "UPDATE cms_parameter_group SET 
					groupName='$groupName',
					active='$active'
				WHERE 
					groupID=$groupID";
	...
	public static function  Delete($oGroup){
		$query = "DELETE FROM cms_parameter_group 

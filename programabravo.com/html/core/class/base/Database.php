...
class Database
{
	...
	protected static function GetResultPaging($query, $limit=25){
		if ($query=="") return false;

		if(self::$orderBy!="") $query.=" ORDER BY ".(self::$orderBy);

		if($limit>0){
			self::setPaging($query, $limit, self::$page);
			$query = $query." LIMIT ".(self::$page).", ".($limit);
		}
		//echo "(".$query.")";
    ...
    private static function setPaging($query, $limit, $page) {
		$result=@mysqli_query(self::Open(), $query);
		self::$_rowCount=self::Count($result);

		$page 	= ($page * $limit);
		$counter= floor((self::$_rowCount/$limit));

		if((self::$_rowCount % $limit)>0) $counter++;
		if($page>($counter*$limit)) $page-=$limit;

...
class CmsSectionLang extends Database
{
	...
	public static function  getItem($sectionID, $langID){
		$query = "
		SELECT 
			sec.sectionID, slan.langID, slan.minisiteID, slan.title, slan.subTitle, slan.description, slan.resumen, slan.media, slan.parameter, slan.metaTag, slan.showContent, slan.staticURL, sec.parentSectionID, sec.sectionName, sec.position, sec.isMinisite, sec.isEditable, slan.active
		FROM 
			cms_section AS sec
	...
	public static function  AddNew_SectionLang($oSectionLang){
		...
		$query = "INSERT INTO cms_section_lang (sectionID, langID, minisiteID, title, description, resumen, media, parameter, metaTag, showContent, staticURL, active)
